...
So I built a first-party shortener: `bitdoze.com/go/<product>/` URLs that redirect to the affiliate destination, fire a Plausible click event on the way out, and live in one JSON file. This site is fully static and hosted on Cloudflare Pages, which supports real HTTP redirects through `_redirects`. I use generated HTML pages here so the browser can send a Plausible event before leaving.
...
| Generated pages (`getStaticPaths`) | Full HTML page → JS redirect | Yes | Yes — best-effort Plausible event | **Affiliate links** |

The trade-off is the middle column. An edge `_redirects` rule is fastest but invisible to analytics — the browser never renders anything, so Plausible can't fire an event. A generated page adds a page load and up to roughly 400ms of JavaScript waiting, allowing a best-effort click event and a visible destination (which affiliate programs want anyway). Since click data is the entire point, `/go/` uses the third option.
...
- **The Plausible queue stub** — same pattern as the site's `Layout.astro`: the `window.plausible` function queues events until the real script loads. Mine is the same custom-domain script (`an3.bitdoze.com`); use the installation snippet and script URL from your own Plausible site settings. The stub is initialized before the async script loads. `autoCapturePageviews: false` keeps these intermediate pages out of ordinary pageview counts while retaining the custom event. Setup details are in the [Plausible on Astro with Cloudflare Workers](https://www.bitdoze.com/astro-plausible-cloudflare-workers/) article.
- **`callback: go` + `setTimeout(go, 400)`** — this is the reliability trick. Plausible calls the callback once the event is sent, so the redirect waits for tracking. The 400ms timeout starts before the event call and bounds the wait. If the tracker is blocked or takes longer than that to load and send, navigation still proceeds and the click may not be recorded. `done` guards against double-firing.
...
Updating a destination URL for the same product is a one-line JSON edit, applied everywhere on the next build. Give a different product its own slug so existing article recommendations stay accurate.
...
## Article attribution and build checks

The event always includes `product`. On this site, the shared layout also adds an `article` query parameter to same-origin `/go/<slug>/` links before interaction, using only the current pathname. This works for ordinary clicks, new tabs, and widgets using `rel="noreferrer"`. The redirect page accepts a same-origin source, strips any query string or fragment, and sends only the pathname as the optional `article` property. Without the parameter, it tries `document.referrer`; direct visits can have no article property. The parameter is never forwarded to the retailer.

For the same behavior, add this script to your shared Astro layout:

```astro
<script>
  function annotateAffiliateLinks() {
    document.querySelectorAll<HTMLAnchorElement>('a[href]').forEach((link) => {
      const target = new URL(link.href, window.location.href);
      if (target.origin !== window.location.origin || !/^\/go\/[^/]+\/$/.test(target.pathname)) return;
      target.searchParams.set("article", window.location.pathname);
      link.href = target.pathname + target.search + target.hash;
    });
  }
  annotateAffiliateLinks();
  document.addEventListener("astro:page-load", annotateAffiliateLinks);
</script>
```

Our build runs `scripts/validate-affiliate-links.mjs` before Astro to check kebab-case slugs, nonempty product names, and HTTPS destinations without embedded credentials. After Astro emits the pages, the same script with `--dist` checks rendered anchors against the map. Checking the output covers both Markdown links and MDX widgets without treating tutorial code samples as real links. Unknown slugs fail the build before deployment. These checks validate configuration and references; they do not check retailer availability.

## Verifying it works

Run `bun run build:ci` for the build and affiliate checks. Then verify the redirects:
...
- A page load plus up to roughly 400ms of JavaScript waiting, versus an edge redirect
</ListCheck>

If any of those become real requirements — especially geographic routing, live editing, or server-side analytics — move the same URL scheme to a Cloudflare Worker or [Sink](https://www.bitdoze.com/sink-install/) and keep `/go/` as the path so no article ever changes.

## Conclusion

This implementation uses generated pages rather than the HTTP redirects also available on Cloudflare Pages. Once you accept that, the implementation is small: one JSON file, one `[product].astro` route, and an event call with a timeout-guarded callback. What you get back is a single place to manage every affiliate URL on the site and click data that tells you which products your readers actually care about.
...
When you need per-click logs, live link editing without a deploy, query passthrough, geographic routing, or server-side analytics. A few hundred small static pages alone is not a reason to migrate. At that point run something like Sink on Cloudflare Workers — you can keep the same `/go/` paths so existing article links keep working.
</Accordion>
