...
This creates the `~/.openfang/` directory with a default `config.toml`, sets up the SQLite database for memory, and walks you through picking an LLM provider.
...
OpenFang uses a `config.toml` file at `~/.openfang/config.toml`. All provider settings go there.
...
OpenFang's provider system recognizes the `glm` keyword and routes it to Zhipu automatically. Internally it handles the correct API format and prefixing.
...
OpenFang detects the `minimax` keyword in the model name and routes requests to the MiniMax API. The provider system handles authentication and request formatting.

### Using both providers

Configure both and switch between them by changing the default model:
...
Change `model` to `"MiniMax-M2.5"` whenever you want to switch. You can also set per-channel model overrides, so Discord could use GLM-5 while Telegram uses MiniMax:
...
The `allow_from` array restricts who can talk to the bot. Leave it empty to let anyone in your server use it, or add specific user IDs. I keep it restricted for personal bots.
...
Send a message in Discord. The bot should respond. The dashboard is also live at `http://localhost:4200`.

## Full config example

Here's what a complete `~/.openfang/config.toml` looks like with GLM-5, MiniMax M2.5 as a fallback, and Discord:
...
|---------|---------|-------------|
| `agents.defaults.model` | — | Which model handles your messages |
...
| `dashboard.port` | `4200` | Port the dashboard listens on |
| `security.wasm_sandbox` | `true` | Run tool code in WASM sandbox |
| `security.audit_trail` | `true` | Merkle hash-chain audit trail for all actions |
...
The dashboard is accessible at `http://your-server-ip:4200` after starting.
...
The dashboard will be live at `http://your-server-ip:4200`. You can add more channel adapters later without restarting — OpenFang hot-reloads config changes for channels.
