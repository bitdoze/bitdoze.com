---
date: 2026-07-02
translationKey: "fish-audio-review"
lastmod: 2026-09-25T00:00:00Z
title: "Fish Audio Review: I Cloned My Voice Free in 15 Seconds"
description: "I put Fish Audio's free plan to work: cloned my voice from a 15s clip, tested S2.1 Pro against ElevenLabs, emotion tags and 83 languages. Honest verdict inside."
image: "../../assets/images/26/07/fish-audio-review.webp"
categories: ["ai"]
authors: ["Dragos"]
tags: ["fish-audio", "voice-cloning", "lip-sync"]
...
Fish Audio is a voice AI platform that does text-to-speech, voice cloning, speech-to-text, and a handful of other audio tasks — plus image and video generation, and a hosted voice-agent platform on top. You can use it through their web app with no code, or through their REST API and Python SDK if you want to build something on top of it.
...
## Fish Creative: image, video, and lip-sync

Fish Audio is no longer just a voice tool. They acquired Thank You AI and folded it into a new suite called **Fish Creative** — same dashboard, same account, now with image generation, video generation, lip-sync, and sound effects on top of the existing TTS and voice cloning.

I tested the video and lip-sync side. Here is what it looks like in practice: you generate a voice (or upload your own audio), pick or generate an image of a character, and Fish Creative syncs the mouth movements to the audio. The result is a talking-head video that you can export and use directly. No After Effects, no separate lip-sync tool, no manual alignment.
...
The lip-sync works with any audio source — your cloned voice, a community voice, or uploaded audio. You can also generate images from text prompts right inside the platform and use those as the base for lip-sync videos. The workflow is: write a script, generate the voice, generate or upload an image, hit lip-sync, export.

I keep coming back to the same thing: this used to take three or four separate tools. HeyGen for the avatar, ElevenLabs for the voice, some image tool for the face, and a video editor to stitch it together. Now it is one tab.

The image generation is decent — not Midjourney quality, but good enough for character portraits and talking-head frames. The video generation handles the lip and jaw movement well. Hair, eyes, and background stay stable. Where it gets weird is extreme head turns or wide-open expressions — the same problem every lip-sync tool has right now.

If you are already using Fish Audio for TTS, the creative suite is worth a look. You are not switching tools, you are just using more of the same one.

<Notice type="info" title="Fish Creative">
Fish Audio now includes image generation, video generation, and lip-sync under the Fish Creative suite. Same account, same dashboard. Free tier includes limited access to the creative tools.
</Notice>

## What shipped recently

Fish Audio has been pushing updates fast. A few additions from the last month or two that are worth knowing about:

**Transcribe 1 Pro (speech-to-text).** Their ASR got a Pro tier aimed at interviews, meetings, and podcasts. It marks speakers inline (`<|speaker:0|>`, `<|speaker:1|>`), keeps emotion cues like `[laughter]` in the transcript, auto-detects the language, and can return word-level timestamps. Same price as the standard model: $0.36 per audio hour. For now it is API-only — set the `model: transcribe-1-pro` header on `POST /v1/asr`. A web app release is on the way.

**Pronunciation dictionaries.** If a name or acronym keeps coming out wrong, you no longer have to paste phoneme tags into every script. Build a dictionary once in the web app (or inline in the API request), publish it, and reference it by ID in TTS calls. English rules use CMU Arpabet, Chinese uses tone-number pinyin, Japanese uses romaji with pitch marks. Up to 3 dictionaries per request, 5,000 rules each.

**The MCP server does images and video now.** Fish Audio's MCP endpoint (`https://api.fish.audio/mcp`) used to be audio-only. An agent connected through Claude, Cursor, or Codex can now search voices, run TTS, transcribe files, and generate or edit images and videos. It signs in with OAuth, spends your plan's package credits rather than API credits, and quotes a price before any image or video job runs — you confirm the cost first. Video generation needs a paid plan.

**Voice Design is free on the web for a limited time.** Describe a voice in plain words — age, accent, tone, pacing — and it generates candidates in about 15 seconds. No sample audio needed. It normally costs 2,000 credits per generation; the API version (`voice-design-1`) stays billed at $0.01 per request.

**Asset Library and Avatars.** Your uploads and generated files now sit in a central Asset Library, and you can save a character as an Avatar inside Image & Video instead of re-uploading the same face every time you lip-sync.

One more thing sitting in the docs: a full **voice-agent platform** — build an agent in a console with a prompt, voice, knowledge base, and tools, then deploy it as a web widget or on a real phone number with inbound calls, outbound dials, and warm call transfers. I have not tested it end to end yet, but it is a much bigger product than the TTS tool I originally signed up for.
...
| Speech-to-text | transcribe-1 / transcribe-1-pro | Scribe |
...
**Content creators** producing videos, podcasts, or audiobooks in multiple languages. The emotion tags make narration sound less robotic, and the pricing will not wreck your budget. If you also need talking-head videos or lip-synced avatars, the Fish Creative suite handles that without leaving the platform.

**Developers** building apps that need TTS. The free S2.1 Pro API is generous and the documentation is decent. Python SDK and REST endpoints are both available, and the MCP server covers transcription plus image and video generation if you want an agent driving it.
...
The Fish Creative suite caught me off guard. Image generation, video, lip-sync — all in the same dashboard I already use for TTS. I used to need HeyGen plus ElevenLabs plus a video editor to make a talking-head clip. Now I can do it in one tab. The lip-sync is not perfect on extreme head movements, but for straight-on talking-head content it holds up. If you are already paying for Fish Audio, test the creative tools before you pay for a separate lip-sync service.
...
- [Fish Audio Image & Video Generation: Lip-Sync, Creative Suite](/fish-audio-image-video-generation/) — the full Fish Creative walkthrough: images, video, lip-sync
