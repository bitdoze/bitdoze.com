...
title: "Clawdbot Setup Guide: Your 24/7 AI Assistant on VPS or Mac Mini"
description: "Complete guide to setting up Clawdbot on Hetzner VPS or Mac Mini. Covers installation, Telegram integration, LLM providers, skills, memory, and configuration."
...
I've been running Clawdbot on a Hetzner VPS for over a week now, connected to Slack and using GLM 4.7 with the coding plan. Recently switched to Gemini CLI with Gemini 3 Flash. Here's what I learned about setting it up and getting real value from it.

Clawdbot is an open-source AI assistant that runs 24/7 on a server and talks to you through Telegram, WhatsApp, Slack, or Discord. Unlike ChatGPT or Claude's web interface, you don't go to it—it comes to you. It remembers conversations across sessions and can proactively message you when things need attention.
...
Most AI tools answer questions when you ask them. Clawdbot does actual work. It can clear your inbox, schedule meetings, research companies, follow up on tasks, and run automations on your computer.
...
The Clawdbot community has created a weird situation: Mac Mini M4 units are actually getting harder to find because people are buying them specifically for Clawdbot. Here's when each option makes sense.
...
I'm using a [Hetzner CX33](https://www.bitdoze.com/hetzner-cloud-review/) (4 vCPU, 8GB RAM) for €5.49/month. It handles Clawdbot comfortably with plenty of headroom for other services.

<Notice type="success" title="Get Started with Hetzner">
[Get €20 credit](https://go.bitdoze.com/hetzner) when you sign up through our referral link. That covers nearly 4 months of running Clawdbot.
</Notice>
...
Install Clawdbot and run the onboarding wizard:
...
Clawdbot supports multiple AI providers. Here's what works and when to use each.
...
If you want to avoid API costs entirely, Clawdbot works with [Ollama](https://www.bitdoze.com/ollama-docker-install/). Install Ollama on your server:
...
Then configure Clawdbot to use local models in your config.
...
6. Paste both into the Clawdbot wizard
...
Scan the QR code from WhatsApp > Settings > Linked Devices. Your personal WhatsApp number becomes the bot's number—messages you send to yourself go to Clawdbot.
...
5. Paste into Clawdbot config

I run Clawdbot in a dedicated Slack channel. Mention @clawdbot to interact.
...
4. Add the token to Clawdbot config

## Memory System

Clawdbot remembers things across conversations. The memory system has a few components:
...
If Clawdbot keeps forgetting something, tell it explicitly:
...
Skills are reusable workflows Clawdbot can run. They extend what it can do beyond basic chat.
...
You can also ask Clawdbot to create skills for you:
...
One of Clawdbot's most powerful skills lets it control AI coding tools directly. If you have Claude Code, OpenAI Codex CLI, or Gemini CLI installed on your server, Clawdbot can orchestrate them to build entire applications.

This means you can message Clawdbot from your phone:
...
Then tell Clawdbot what's available:

> "I have Claude Code installed. When I ask you to build something, use it to write the code."

Clawdbot stores this in memory and uses the appropriate tool when you request coding tasks. This effectively turns your $20/month Claude subscription into an on-demand development team you can command from Telegram.
...
Clawdbot runs cron jobs for scheduled tasks. This is where it becomes more than a chatbot—it's an automation engine.
...
Clawdbot stores these and proactively messages you at the scheduled time.

### Cron-style Scheduling

For more complex schedules, Clawdbot supports cron expressions:

> "Run a disk space check every day at midnight."

> "Every 6 hours, check if my website is responding and alert me if it's down."

Behind the scenes, Clawdbot uses the server's cron system. You can view scheduled jobs:
...
Unlike ChatGPT or Claude web, Clawdbot messages you first. Set up monitoring:
...
## Enhancing Clawdbot with Agno and Python Scripts
...
Clawdbot handles messaging and orchestration. Agno handles complex AI workflows—multi-agent teams, RAG pipelines, tool use. Together:

- Clawdbot receives your Telegram message
- Clawdbot runs your Agno script
- Agno does the heavy lifting (web research, document analysis, multi-step reasoning)
- Results come back through Clawdbot
...
Create a scripts directory for Clawdbot:
...
Create a research agent that Clawdbot can invoke:
...
Clawdbot runs your Agno script, which spawns its own AI agent to do web searches, and returns the results.
...
Clawdbot runs with access to your server. A few things to keep in mind:
...
Once you have Clawdbot running:
...
**Can multiple people use one Clawdbot?**
...
Clawdbot isn't magic—it's a framework for running AI assistants that integrate into your existing workflow. The setup takes some effort, but having an always-available assistant that actually does things rather than just talking about them has been worth it for me.
