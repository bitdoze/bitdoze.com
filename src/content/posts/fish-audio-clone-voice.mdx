...
title: "Clone Your Voice with Fish Audio in 2 Minutes (2026 Guide)"
description: "Clone your voice from 15 seconds of audio on Fish Audio's free tier. Step-by-step with recording tips, emotion tags, screenshots, and mistakes that ruined my…"
...
Sometimes the clone is fine but one word gets mangled — a name, a brand, an acronym like SQL. The old fix was editing the script with phoneme tags every time. Fish Audio now has pronunciation dictionaries: define word-to-phoneme rules once in the web app (under **Pronunciation Dictionary** in the dashboard), publish a version, and it applies to every generation after that. English rules use CMU Arpabet, so `Kubernetes` becomes `K UW2 B ER0 N EH1 T IY0 Z`. Chinese and Japanese have their own phoneme formats. If you generate through the API, pass the published dictionary ID in the request, or send an inline dictionary for a one-off fix.

## No mic? Use Voice Design instead

If recording a clean 15 seconds is the blocker, Voice Design builds a voice from a text description — age, accent, tone, pacing — in about 15 seconds, no audio required. It is free on the website during launch (normally 2,000 credits per generation). The result is a voice model you use exactly like a clone. It will not be *your* voice, which is the whole point of cloning, but for a character voice or a scratch narration track it is the faster path.
