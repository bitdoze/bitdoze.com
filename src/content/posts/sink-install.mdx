...
<Picture formats={["webp"]} fallbackFormat="webp" src={img1} alt="Cloudflare Workers bindings configuration for Sink" />
...
<Picture formats={["webp"]} fallbackFormat="webp" src={img2} alt="Sink dashboard showing the link management UI" />
...
If you're building out your self-hosted stack, Sink pairs well with [Docker containers for your home server](/docker-containers-home-server/) — one runs on your infrastructure, the other runs free on Cloudflare's edge. Sink is one of [29 self-hosted apps on Cloudflare Workers](/self-hosted-apps-cloudflare-workers//) we've cataloged, including email clients, password managers, and note-taking apps.
