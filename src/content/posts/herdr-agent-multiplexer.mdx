...
This review covers what Herdr is, how it compares to tmux and Zellij, how to install and configure it, and whether it actually delivers on its promise of terminal-native agent multiplexing. It includes a hands-on video walkthrough of how I use it day to day. If you're exploring [AI coding tools and assistants](/ai-coding-tools/), this will help you decide if Herdr fits your workflow.
</Notice>
...
## How I'm actually using Herdr (video walkthrough)
...
I recorded a full walkthrough of my daily Herdr setup. The short version: I use a small slice of what Herdr can do, and that slice is why it stays open all day.

**It replaced tmux for me — because of the mouse.** I never got comfortable with tmux because I like working with the mouse, not only the keyboard. Herdr gives you spaces and panes you can add by clicking, resize by dragging, and close from a menu. Key bindings and tabs are there too, and both input styles work side by side.

**I run the defaults and nothing else.** The documentation covers integrations, plugins, a file viewer, Neovim bindings, workflows. I use none of it. My whole loop is: run `herdr`, get a clean workspace, start an agent in a pane.

**Agents are detected with zero config.** When I start an [Oh My Pi (OMP)](https://omp.sh/) instance in a pane, Herdr picks it up automatically and notifies me the moment the agent needs my input.

**Agents can drive Herdr itself.** In the video I ask OMP to open a panel and check free disk space. It reads the Herdr skill (installed with one command), spins up its own pane, and runs the commands without me touching anything. Later in the video it opens a second OMP instance in a new panel and sends work to it. That's the orchestration API from the next section, reached through plain conversation.

**Closing the window changes nothing.** This is the part I care about most: close the terminal mid-task and the agent keeps working. Run `herdr` again and everything is where you left it — same spaces, same panes, same agents still running.

**My agents live on a VPS.** I run `herdr --remote` with an SSH config alias (the exact setup from the [remote attach section](#remote-ssh-attach)), so my projects on the server are always alive. Laptop shut, out of power, on the road — the work continues on the VPS and I attach from whatever device is in front of me. The day before recording, I burned a couple of million tokens on a project update while barely watching it run.

**There's also a desktop app.** Herdr M (macOS) wraps the same sessions in a regular UI: all your spaces, file access, and remote connections in one window. Devices are linked, so when I close Ghostty mid-task I can pick the same session up in the app. I'm a terminal person so I rarely use it, but it's the friendliest way in if the TUI isn't your thing.

Day to day that's Ghostty locally, and on the VPS a TUI file manager with Herdr nested inside. None of it is required — it's just what stuck. If you use Herdr differently, especially the plugin ecosystem, tell me in the comments on the video.
...
Herdr pairs with coding agents covered in [top AI GitHub repos](/top-ai-github-repos/) (OpenCode, Pi, Cline, OpenHands).
