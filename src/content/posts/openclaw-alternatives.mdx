...
description: "A look at NanoClaw, nanobot, memU, bitdoze-bot, PicoClaw, and IronClaw as self-hosted alternatives to OpenClaw for running your own 24/7 AI assistant."
...
OpenClaw (the project that went throughs Clawdbot and Moltbot name changes) made a lot of people realize they could run an AI assistant on their own server. Always on, always reachable through Telegram or Slack, and not dependent on anyone's SaaS. I've been running it myself and wrote a [full setup guide](/clawdbot-setup-guide/) if you want to try to original.
...
| | NanoClaw | nanobot | memU | bitdoze-bot | PicoClaw | IronClaw |
|---|---|---|---|---|
| **GitHub stars** | ~10 | 15.4k | 8.7k | 7 | 1.1k | 368 |
| **Language** | Python | Python | Python + Rust | Python | Go | Rust |
| **Codebase size** | ~3k lines | ~3.5k lines | Larger (framework) | Medium | Small (single binary) | Medium-large |
| **License** | MIT | MIT | Apache 2.0 | MIT | MIT | Apache 2.0 / MIT |
| **Chat channels** | Telegram | Telegram, Discord, WhatsApp, Slack, Feishu, DingTalk, Email, QQ | Bot at memu.bot | Discord | Telegram, Discord | REPL, HTTP, Telegram, Slack (WASM) |
| **Memory** | SQLite | Built-in | Hierarchical (main feature) | Agno memory + learning | File-based workspace | PostgreSQL + pgvector (hybrid search) |
| **Install method** | Docker / manual | pip (`nanobot-ai`) | pip (`memu-py`) | UV + manual | Single binary / source | cargo build |
| **Local models** | Via OpenRouter | vLLM support | Via custom providers | OpenAI-compatible | Via OpenRouter | Via NEAR AI |
| **Security features** | FileGuard, ShellSandbox, PromptGuard | Basic | N/A (framework) | Tool permissions, audit | Basic | WASM sandbox, credential protection, prompt injection defense |
| **Multi-agent** | No | No | No (memory layer) | Yes (Agno teams) | No | Parallel jobs with isolated workers |
...
nanobot init
# Follows wizard to configure channels and API keys
nanobot start
```
...
<Button text="GitHub Repository" link="https://github.com/theonlyhennygod/zeroclaw" variant="solid" color="purple" size="md" icon="github" />
...
New memories get auto-categorized. Related memories link to each other. The system claims 92% accuracy on the Loco benchmark, which tests how well memory systems retain and retrieve information over long conversations.
...
- Endpoint allowlisting so that agent can only reach hosts you approve
