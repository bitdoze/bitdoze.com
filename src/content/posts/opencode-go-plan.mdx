...
description: "OpenCode Go bundles 14 models including DeepSeek V4 Pro, Qwen 3.7, and MiniMax M3 for $10/mo. Rate limits, model list, and how to use it with any AI coding tool."
...
OpenCode Go is a subscription from the OpenCode team that gives you access to 14 open source models through a single API key. $10/month (first month $5), no per-token billing to track, no separate accounts with model providers.
...
| Qwen 3.7 Plus | Newest Qwen, improved over 3.6 |
| Qwen 3.7 Max | High-end Qwen for complex tasks |
| Qwen 3.6 Plus | Previous generation, still strong |
| Kimi K2.5 | Moonshot's model, good with Chinese and English |
| Kimi K2.6 | Newer version with better benchmarks |
| MiniMax M3 | Newest MiniMax, improved reasoning |
...
For someone just getting started with AI coding agents, though, Go is a no-brainer. Instead of signing up for five different model providers, copying five API keys, and tracking five billing dashboards, you get one key that works with 14 models. And you can use that key across every agent you run.
