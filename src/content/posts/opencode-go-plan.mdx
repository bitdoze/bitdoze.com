...
description: "OpenCode Go bundles 12 open source models like DeepSeek V4, Qwen 3.6, GLM 5.2, and MiniMax M3 for $10/mo. Use it with OpenCode, Hermes Agent, OpenClaw, or any AI coding tool."
...
The best part: you do not need to use OpenCode to use Go. The API endpoint is OpenAI-compatible, so you can plug it into [Hermes Agent](/hermes-agent-setup-guide/), [OpenClaw](/clawdbot-setup-guide/), [Pi Agent](/pi-coding-agent-setup-guide/), [Mastra](/build-ai-agent-mastra/), Agno, or any other tool that talks to an LLM API. One subscription, one API key, works everywhere.
...
| Model | Notes |
|-------|-------|
| **GLM 5.2** | Zhipu's flagship, strongest open-source for coding (62.1% SWE-Bench Pro, 81.0% Terminal-Bench) |
...
<Notice type="info" title="GLM 5.2 coming soon">
GLM 5.2 is the strongest open-source model released in 2026, with 81.0 on Terminalbench and a 1M token context window. It is rolling out to OpenCode Go members. Check `/models` in OpenCode for availability.
</Notice>
...
If you are doing heavy agentic work with GLM 5.2, you might hit the 5-hour limit during a long session. If you are using DeepSeek V4 Flash or Qwen 3.5 Plus, you are unlikely to hit limits unless you are running agents 24/7.
...
For someone just getting started with AI coding agents, Go is a no-brainer. Instead of signing up for five different model providers, copying five API keys, and tracking five billing dashboards, you get one key that works with 12 models. And you can use that key across every agent you run.

If you are curious, start with the first month at $5. Use GLM 5.2 for complex coding tasks, MiniMax M3 for everyday agentic work, and DeepSeek V4 Flash for quick tasks where you want faster responses.
