...
description: "Practical list of 125+ Docker containers for a home server in 2026: media, VPN, PaaS, monitoring, finance, and more."
...
The containers below run fine on almost anything, but if you are building a box from scratch these three pieces cover the usual homelab needs:
...
For more options, see the [best mini PCs for a home server](/best-mini-pc-home-server/) guide.
...
Worth knowing: one PostgreSQL instance can cover several of these categories at once. See [what Postgres can replace](/what-postgres-replaces/) before deploying a separate cache or search container.
