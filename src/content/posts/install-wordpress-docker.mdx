...
> This being said we will get started and install WordPress in a Docker with docker-compose, I will use the `latest` for the image tags but you can use the exact version if you don't want surprises when a container moves to a next version. If you are running WooCommerce, you can also deploy the [Woo Admin product dashboard](https://www.bitdoze.com/woocommerce-admin-dashboard/) alongside WordPress in Docker for faster product management.
...
- VPS where you can host WordPress, you can use one from [Hetzner](https://go.bitdoze.com/hetzner), [Hostinger](https://go.bitdoze.com/hostinger-vps) or use a [Mini PC as Home Server](https://www.bitdoze.com/best-mini-pc-home-server/)
