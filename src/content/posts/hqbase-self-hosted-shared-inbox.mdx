...
HQBase ships an OAuth-protected MCP server, so [AI coding tools like Claude Code and Cursor](https://www.bitdoze.com/ai-coding-tools/) can search, read, draft, and send email on your behalf.
...
For teams already on Cloudflare who want to eliminate mail-server ops and flatten per-seat email costs, it's worth evaluating. Start with read-only MCP, run `doctor` regularly, and back up before every change. HQBase is one of [29 self-hosted apps that run entirely on Cloudflare Workers](/self-hosted-apps-cloudflare-workers/) — password managers, note-taking apps, analytics, and more, all on the same serverless platform.
