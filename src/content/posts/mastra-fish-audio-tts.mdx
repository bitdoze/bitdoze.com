...
One newer request field worth knowing: `pronunciation_dictionary`. If a word keeps coming out wrong — your brand name, "Mastra" itself, acronyms like SQL — you used to have to edit phoneme tags into every script the agent writes. Now you can publish a dictionary once in the web app and pass `{ "id": "...", "version": "..." }`, or send an inline dictionary for a one-off:

```json
{
  "text": "Mastra agents call Kubernetes jobs.",
  "reference_id": "YOUR_VOICE_MODEL_ID",
  "pronunciation_dictionary": [
    {
      "items": [
        { "key": "Kubernetes", "value": "K UW2 B ER0 N EH1 T IY0 Z", "case_sensitive": false }
      ]
    }
  ]
}
...
English entries use CMU Arpabet; Chinese uses tone-number pinyin; Japanese uses romaji with pitch marks. Max 3 dictionaries per request, 5,000 rules each. Full reference: [pronunciation dictionaries](https://docs.fish.audio/developer-guide/core-features/fine-grained-control/pronunciation-dictionaries).
...
### Or skip the tool: Fish Audio MCP

If you do not want to maintain a custom tool at all, Fish Audio ships an MCP server at `https://api.fish.audio/mcp` (streamable HTTP, OAuth sign-in). Connect it to a Mastra agent's MCP toolset — or to Claude Code, Cursor, or Codex — and the agent can search voices, run TTS, transcribe files, and, since the latest update, generate and edit images and videos.

Two trade-offs to know before swapping `fish_tts` for it: MCP spends your plan's package credits rather than developer API credits (image and video jobs quote a price and ask the user to confirm), and it returns hosted URLs instead of writing a `narration.wav` into your workspace. For a scripted video pipeline I still prefer the deterministic tool. For quick "say this in my voice" access from any MCP client, the server is the zero-code path. Same trade-off I covered in [Mastra tools vs MCP](/mastra-tools-vs-mcp/).
...
Fish Audio has its own ASR if you want one vendor end to end: `POST /v1/asr` returns the text plus timestamped segments. The newer `transcribe-1-pro` model (select it with the `model` HTTP header) is built for multi-speaker recordings — interviews, meetings, podcasts — with inline `<|speaker:N|>` markers and emotion cues like `[laughter]` preserved in the transcript. Both models cost $0.36 per audio hour.
