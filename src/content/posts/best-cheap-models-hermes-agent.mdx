...
If you do not want to manage separate API keys and billing for each provider, [OpenCode Go](https://go.bitdoze.com/opencode-go) bundles all five models into a single subscription. For a detailed review with limits and real-world usage, see the [OpenCode Go review](/opencode-go-review/).
...
Yes. OpenCode Go provides an OpenAI-compatible API endpoint. Set the base URL to `https://opencode.ai/zen/go/v1/chat/completions` in your Hermes config and use your OpenCode Go API key. At $5 for the first month and $10/month after, it bundles all five models listed here. For a detailed review with limits and benchmarks, see the [OpenCode Go review](/opencode-go-review/).
</Accordion>
