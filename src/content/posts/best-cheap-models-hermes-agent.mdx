...
For the full Hermes setup chain: start with the [installer](/hermes-agent-setup-guide/), set up a [dashboard](/best-hermes-dashboards/) for browser access, and configure the [built-in web UI](/hermes-dashboard-guide/) if you want SSH-tunneled access. If you want to try free models first, the [Nous Portal guide](/hermes-agent-mimo-v2-pro/) covers the free promotions that rotate through Hermes partnerships.
