---
date: 2026-07-02
translationKey: "fish-audio-vs-minimax"
...
Ambas plataformas ofrecen APIs REST, aunque la de Fish Audio ha crecido mas alla del texto a voz. La experiencia de desarrollador difiere en algunas formas.
...
- Tambien voz a texto: `transcribe-1` y `transcribe-1-pro` (marcas de hablante, cues de emocion), $0.36/hora de audio
- Diccionarios de pronunciacion via app web o API
- Servidor MCP que cubre TTS, transcripcion y generacion de imagen/video
...
Una diferencia mas a notar: la API publica de audio de MiniMax se centra en TTS — no hay endpoint de voz a texto self-serve a dia de hoy. Fish Audio ya cubre ambas direcciones — `transcribe-1` para transcripcion simple y `transcribe-1-pro` para reuniones multi-hablante con marcas de hablante en linea — ademas de diccionarios de pronunciacion y un servidor MCP que llega a las herramientas de imagen y video. Si tu producto necesita STT junto a TTS, MiniMax no lo ofrece.
