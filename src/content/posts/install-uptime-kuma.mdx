...
<Picture formats={["webp"]} fallbackFormat="webp" src={img1} alt="Uptime Kuma add monitor screen" />
...
<Picture formats={["webp"]} fallbackFormat="webp" src={img2} alt="Uptime Kuma SMTP notification setup" />

Once configured, the dashboard shows a green heartbeat for every healthy monitor:

<Picture formats={["webp"]} fallbackFormat="webp" src={img3} alt="Uptime Kuma dashboard showing all monitors up" />

When something goes down you receive an alert email, and another one when it recovers:

<Picture formats={["webp"]} fallbackFormat="webp" src={img4} alt="Uptime Kuma downtime notification email" />
