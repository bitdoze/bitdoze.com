...
<Notice type="info" title="Level up: persistent agent memory">
If you need your Agno agent to remember facts and learn across sessions (not just within one), check out [Hindsight](/hindsight-docker-deploy/) — it has a [direct Agno integration](https://hindsight.vectorize.io/integrations) that replaces SQLite chat history with structured long-term memory, entity extraction, and semantic search.
</Notice>
